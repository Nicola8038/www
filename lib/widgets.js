...
function loadTokenData(token) {
  ...
  fetch(aktionariatURL + "token?ticker=" + token).then(function (response) {
    response.json().then(function (json) {
      data.setVariable("totalshares", json.totalShares.toLocaleString().replace(",", " "));
      data.setVariable("totalsupply", json.totalSupply.toLocaleString().replace(",", " "));
      data.setVariable("basesupply", json.baseSupply.toLocaleString().replace(",", " "));
        ...
        response.json().then(function (json1) {
          ...
          fetch(aktionariatURL + "treasury?ticker=" + token).then(function (response) {
            response.json().then(function (json2) {
              ...
              data.setVariable("marketcap", ((json.totalShares - json2.treasuryShares) * json1.price).toLocaleString().replace(",", " ") + " " + json1.base);
            });
          });
        });
      });
