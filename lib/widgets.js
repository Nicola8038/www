...
class AktTrades extends AktTickerElement {
  connectedCallback() {
    ...
    table.setAttribute("data-url", "http://127.0.0.1:8080/trades?ticker=DAKT");
    table.innerHTML =
    '<thead><tr><th data-field="datetime" data-formatter="dateFormatter">Time</th><th data-field="type">Type</th><th data-field="sharesTraded">Shares Traded</th><th data-field="pricePerShare">Price</th><th data-field="insiderMessage" data-formatter="insiderFormatter">Note</th></tr>';
...
function insiderFormatter(value, row) {
  if (row.insiderMessage == null){
    return "";
  } else if (row.insiderMessage.length < 10){
    return row.insiderMessage;
  } else {
    return row.insiderMessage.substring(0, 8) + "...";
  }
}

function detailFormatter(index, row) {
  var part1 = 'This trade was done in block <a target="_blank" href="https://etherscan.io/tx/' + row.transactionHash + '">' + row.block + '</a>. ';
  var part2 = 'The ' + (row.type == 'buy' ? "buyer" : "seller") + ' paid a price of ' + row.pricePerShare + ' ' + row.currency + ' per share. ';
  /* if (row.pricePerShare != row.endPrice) {
    part2 += 'Afterwards, the new market price was ' + row.endPrice + ' ' + row.currency + '. ';
  } */
  if (row.insiderMessage != null){
    return part1 + part2 + 'The trader declared to be an insider and provided the following motivation: "' + row.insiderMessage + '"';
  } else {
    return part1 + part2;
  }
}
