...
function loadTokenData(token) {
  ...
  fetch(aktionariatURL + "token?ticker=" + token).then(function (response) {
    response.json().then(function (json) {
      data.setVariable("totalshares", json.totalShares.toLocaleString().replaceAll(",", " "));
      data.setVariable("totalsupply", json.totalSupply.toLocaleString().replaceAll(",", " "));
      data.setVariable("basesupply", json.baseSupply.toLocaleString().replaceAll(",", " "));
      ...
      fetch(aktionariatURL + "price?ticker=" + token).then(function (response) {
        response.json().then(function (json1) {
          data.setVariable("price", json1.price + " " + json1.base);
          fetch(aktionariatURL + "treasury?ticker=" + token).then(function (response) {
            response.json().then(function (json2) {
              data.setVariable("treasuryshares", json2.treasuryShares);
              data.setVariable("marketcap", ((json.totalShares - json2.treasuryShares) * json1.price).toLocaleString().replaceAll(",", " ") + " " + json1.base);
            });
          });
        });
      });
...
class AktPriceLine extends AktTickerElement {
  connectedCallback() {
    var container = document.createElement("div");
    container.setAttribute("id", "chart_price_line_div");
    this.appendChild(container);
  ...
  drawPriceLineChart() {
    var chart = new google.visualization.LineChart(document.getElementById('chart_price_line_div'));
